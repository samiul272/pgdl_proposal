...
from ingestion import input_dir
...
def check_data_validity_v2(cnf):
    params = next(iter(cnf.values()))['hparams']
    expected_names = itertools.product(
        *[[(hparam_name, value) for value in params[hparam_name]['possible_values']] for hparam_name in params])
    expected = collections.Counter(expected_names)
    found_names = [tuple((hparam_name, e['current_value']) for hparam_name, e in v['hparams'].items()) for v in
                   cnf.values()]
    ...
    for i, basename in enumerate(data_names):
        ...
        try:
            ...
            if len(model_specs) != len(prediction):
                raise ValueError(
                    "Prediction shape={} instead of Solution shape={}".format(len(prediction), len(model_specs)))
            try:
