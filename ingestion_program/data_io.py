...
# Functions performing various input/output operations for the NeurIPS PGDL competition
# Main contributors: Arthur Pesah and Isabelle Guyon, August-October 2014
# Modified: Yiding Jiang, July 2020
...
def inventory_data_dir(input_dir):
    ...
    training_names = ls(
        input_dir + '/*/*_train1.data')  # This supports subdirectory structures obtained by concatenating bundles
    for i in range(0, len(training_names)):
        name = training_names[i]
        training_names[i] = name[-name[::-1].index(filesep):-name[::-1].index('_') - 1]
    ...
    with open('performance.csv', 'a') as fp:
        ...
        data = [
            ...
            [basename, n_estimators, platform.system(), platform.machine(), platform.platform(),
             float("{0:.2f}".format(mem_used / 1048576.0)), float("{0:.2f}".format(time_spent)), time_budget]
        ]
        a.writerows(data)
