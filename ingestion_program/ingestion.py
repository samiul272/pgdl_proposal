...
if __name__ == "__main__" and debug_mode < 4:
    ...
    from baselines.proposed_complexity.complexity import complexity  # complexity measure
    ...
    for basename in datanames:  # Loop over datasets
        ...
        # (write a new data manager for loading the models)
        ...
        for mid in D.model_ids:
            ...
            try:
                ...
                raise TypeError(
                    'Measure should be a scalar float or numpy float but got type: {}'.format(type(measure_val)))
        ...
        if time_exceeded:
            vprint(verbose, "[+] Time exceeded: time limit is {} but program has run for {}".format(time_budget,
                                                                                                    time.time() - start))
        else:
